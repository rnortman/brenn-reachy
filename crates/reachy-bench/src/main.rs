...
//! motion commands.
//!
//! Everything else in this workspace is a library that owns no loop. This binary
...
fn main() -> anyhow::Result<()> {
    // A bench tool that exits zero having done nothing is worse than one that
    // refuses: the operator would read silence as a pass.
    anyhow::bail!("reachy-bench: no commands are implemented yet")
}
