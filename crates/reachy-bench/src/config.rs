...
mod tests {
    ...
    fn the_motion_layers_profile_units_are_the_wire_layers() {
        ...
        for (group, plant, pair) in [
            ...
            assert_eq!(
                plant.v_max,
                dxl_proto::conv::profile_velocity_rad_per_s(pair.velocity) * period_s,
                "{}",
                group.name()
            );
            assert_eq!(
                plant.a_max,
                dxl_proto::conv::profile_acceleration_rad_per_s2(pair.acceleration)
                    * period_s
                    * period_s,
                "{}",
                group.name()
